...
def SHC_on_one_channel(data, domain, freqs = None):
    """
    ...
    """
    if domain == 'time':
        freq, ampl = fft_of_one_channel(data, SHC_LOW_FREQ, SHC_HIGH_FREQ)
...
def shc_calc(frequencies, amplitudes, jump):
    ...
    for i in range(0, len(amplitudes), jump):
        ...
        for r in range(2, SHC_HARMONICS + 1):
            ...
            first_indice = np.where(frequencies == freq_values[0])[0]
            closest_indices = np.arange(first_indice, first_indice + indices_len, 1)
