...
import numpy as np
from ..basic_tools import fft_of_all_channels, normalize_array
...
def hps_on_one_channel(frame, domain, freqs = None):
    """
    ...
    f0_per_channel = []
    if domain == 'time':
        fft_freq_array, fft_channel_result = fft_of_all_channels(frame, HPS_LOW_FREQ, HPS_HIGH_FREQ)
...
def hps_on_all_channels(frames, domain, freqs = None):
    """
    ...
    for i in range(len(frames)):
        f0_per_channel.append(hps_on_one_channel(frames[i], domain, freqs))
    return f0_per_channel