...
from One_data_frame_processes.Localization.f0_methods.HPS_method import hps_on_one_channel
from One_data_frame_processes.Localization.Power.power_around_frequencies import find_power_around_frequencies
from One_data_frame_processes.Localization.Power.total_power_of_node import find_total_power_of_node
from One_data_frame_processes.Localization.f0_methods.SHC_method import SHC_on_one_channel
from One_data_frame_processes.Localization.f0_methods.Erez_method import perform_erez_method_on_one_channel
...
def calculate_ratio_of_lobes(max_pair, amplitudes, frequencies, power_array):
    ...
    if RATIO_CALCULATION_METHOD == SELECTED_LOBE_METHOD:
        ...
        # get list of f0s
        if F0_METHOD == 'HPS':
            f0 = hps_on_one_channel(amplitudes[max_pair[0]], 'frequency', frequencies)
        elif F0_METHOD == 'SHC':
            f0 = SHC_on_one_channel(amplitudes[max_pair[0]], 'frequency', frequencies)
        elif F0_METHOD == 'EREZ':
